...
from services.backup_service import collect_backup_data, restore_json_backup
...
def test_restore_json_backup_success(tmp_path):
    ...
    assert result["backup_version"] == "1.0"
...
def test_restore_is_atomic_on_database_failure(tmp_path):
    ...
    if not accounts or not categories:
        pytest.skip("Accounts and categories are required for atomicity test.")
    ...
    with pytest.raises(RuntimeError, match="Atomic backup restore failed"):
        restore_json_backup(backup_file)
